...
// GET /api/applicants?archived=false
...
// POST /api/applicants
// action: 'insert' | 'update' | 'delete' | 'status' | 'import' | 'archive' | 'cross_reference'
export async function POST(req) {
  ...
  if (action === 'delete_batch') {
    const { ids, label, count } = payload
    const { error } = await supabase.from('applicants').delete().in('id', ids)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await logActivity('Deleted archive batch', 'archive', label, `Deleted ${count} archived applicants`)
    return NextResponse.json({ success: true })
  }
  ...
  if (action === 'archive') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await logActivity('Archived applicants', 'applicant', label, `Archived ${count} applicants`)
    return NextResponse.json({ success: true })
  }

  if (action === 'cross_reference') {
    // Runs the full cross-reference logic server-side
    ...
    for (const vs of visitStudents) {
      ...
      if (match) {
        await supabase.from('matches').upsert(
          { visit_student_id: vs.id, applicant_name: match.full_name, applicant_email: match.email, applicant_phone: match.phone },
    ...
    // Sync saved matches back to applicants
    ...
    if (savedMatches) {
      for (const m of savedMatches) {
        ...
        if (applicant) {
          await supabase.from('applicants').update({ is_matched: true, matched_visit_student_id: m.visit_student_id }).eq('id', applicant.id)
        }
      }
    }
