  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#ffffff', margin: '0 0 4px 0', letterSpacing: '-0.5px' }}>Pending Applicants</h1>
          <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', margin: 0 }}>Manage all pending applicants and their status</p>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
        ...
        <select value={filterNationality} onChange={(e) => setFilterNationality(e.target.value)} style={s.select}>
          <option value="all" style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>All Nationalities</option>
          {nationalities.map(n => <option key={n} value={n} style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>{n}</option>)}
        </select>
                ...
                >
                  ...
                  <td style={s.td}>
                    <select value={applicant.status} onChange={(e) => handleStatusChange(applicant.id, e.target.value)} style={{ background: 'transparent', border: 'none', fontSize: '12px', fontWeight: '600', cursor: 'pointer', color: applicant.status === 'green' ? '#10b981' : '#ef4444', outline: 'none' }}>
                      <option value="red" style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>Red</option>
                      <option value="green" style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>Green</option>
                    </select>
                  </td>
                  <td style={s.td}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                <select value={form.paid} onChange={(e) => setForm({ ...form, paid: e.target.value === 'true' })} style={s.input}>
                  <option value="false" style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>No</option>
                  <option value="true" style={{ backgroundColor: '#1a1a2e', color: '#ffffff' }}>Yes</option>
                </select>
              </div>
              <div>
